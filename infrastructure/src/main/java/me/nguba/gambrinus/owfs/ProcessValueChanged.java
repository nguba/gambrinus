...
public final class ProcessValueChanged extends MutatorEvent
{
    ...
    public static ProcessValueChanged from(final Instant instant, final OneWireAddress address, final Temperature expected)
    {
        if (address == null) {
            throw new IllegalArgumentException("OneWireAddres cannot be null.");
        }
        
        return new ProcessValueChanged(instant, address, expected == null ? Temperature.celsius(0) : expected);
    }
    ...
    private ProcessValueChanged(final Instant instant, final OneWireAddress adddress, final Temperature processValue)
    {
    	super(instant);
	...
	public String toString() {
		return "ProcessValueChanged [processValue=" + processValue + ", address=" + address + ", timestamp=" + timestamp
				+ "]";
	}
